defmodule RumblWeb.VideoControllerTest do
  ...
  # verifica que no se pueda acceder a dichas paginas sin estar
...
#---Check if another user can update, show, delete another user's video
  test "authorizes actions against access by other users", %{conn: conn} do
    ...
    assert_error_sent :not_found, fn -> #check if the error is a 404 / not_found
  ...
  # -----------test for a logged-in users-----------------------

  # we use tags to mark attributes for a block of test, exluding others

  describe "With a logged-in user" do
    setup %{conn: conn, login_as: username} do
      # create an user
      user = user_fixture(username: username)
      # assing the user to the connection
      conn = assign(conn, :current_user, user)
      {:ok, conn: conn, user: user}
    end

    # we can call this test using its tag
    @tag login_as: "max"
  ...
  test "does not create vid, renders errors when invalid", %{conn: conn} do
    ...
    conn =
      post conn, Routes.video_path(conn, :create),
        video: @invalid_attrs(assert html_response(conn, 200) =~ "check the errors")
